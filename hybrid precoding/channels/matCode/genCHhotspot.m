function CH = genCHhotspot(K,NT,r)
% range of Outdoor hotspot changes
        ...
        case {"3GPP_38.901_UMi", "3GPP_38.901_UMa", "3GPP_38.901_RMa"} 
            switch no_sector
                case 1/3
                    ...
                    rho = sqrt(x.^2 + y.^2);          
                    theta = atan2(y, x);         
                    theta_compressed = mod(theta, 2 * pi / 4) - pi / 4;
                    ...
                    % Place all users near the first user with a radius of r
                    if r ~= inf
                        qd_l.rx_position(3,:) = ones(1,no_rx) * qd_l.rx_position(3,1); %hights
                        theta = 2*pi*rand(1,no_rx);     
                        rho = r * sqrt(rand(1,no_rx));   
                        qd_l.rx_position(1,:) = ones(1,no_rx) * qd_l.rx_position(1,1) + rho .* cos(theta); 
                        qd_l.rx_position(2,:) = ones(1,no_rx) * qd_l.rx_position(2,1) + rho .* sin(theta);
                    end
