function CH = genCHfc(K,NT,fc)
% Carrier freq. changes
...
%% Settings
    ...
    if fc > 6e9
        ...
        SCS = 60e3; % Subcarrier spacing in [Hz]
        no_RB = 132; %Maxium number of RBs, determined by SCS and BW as described in TS 38.101-2
    else
    ...
    % aBS  = qd_arrayant( '3gpp-mmw', 1, NT, fc, 1, [], 0.5, 1, 1, 2.5, 2.5); %type, M, N, fc, Polarization indicator, downtilt in [deg] (only for indicator>=4), element spacing in [lambda], Mg, Ng, panel V-spacing in [lambda], panel H-spacing in [lambda]
    aBS  = qd_arrayant( '3gpp-mmw', 2, 2, fc, 3, [], 0.5, 1, 2, 2.5, 2.5); % 16 antennas %type, M, N, fc, Polarization indicator, downtilt in [deg] (only for indicator>=4), element spacing in [lambda], Mg, Ng, panel V-spacing in [lambda], panel H-spacing in [lambda]
    ...
    aMT = qd_arrayant('omni'); % single antenna
    ...
    simulated_subcarrier = 1; % only use one subcarrier, because narrow-band system considered
    Nsubcarrier = 1; % only use one subcarrier
    ...
    CH = squeeze(CH) * 10^(-noiseVariancedBm/20); % scale up the channel coefficients to avoid precision loss from floating-point number
end