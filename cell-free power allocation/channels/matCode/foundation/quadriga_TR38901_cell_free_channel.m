% simulate cell-free downlink

% output is frequency-domain chanel CH with size [NR, NT, simulated_RB, simulated_slot, K, L]
% if static, the size is [NR, NT, simulated_RB, K, L]
...
scenario_type = scenario_set(1); %choose one from 1~5

width_area = 500; % Width of the area in[m]
length_area = 500; % Length of the are in[m]

no_AP = 25; % Number of APs randomly located in the area, denoted as L
no_rx = 10; % Number of MTs (i.e., Users / UEs)  randomly located in the area, denoted as K
...
%% Plot map 
[ map,x_coords,y_coords] = qd_l.power_map('3GPP_38.901_UMi_LOS', 'quick',5,-width_area,width_area*2,-length_area,length_area*2,UE_height); % plot with a LOS channel for example, this is NOT the outputted channel
P = 10*log10( sum(cat(3,map{:}), [3,4])); % received power, both transmit and receive (rx_array of the 1st rx) array gains considered
qd_l.visualize([],[],2); hold on   % Show BS and MT positions
imagesc( x_coords, y_coords, P ); % Plot the received power
axis([-width_area,width_area*2,-length_area,length_area*2])
